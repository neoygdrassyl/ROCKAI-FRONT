    ...
    const columns = [
        {
            ...
            text: row => row.codigo,
            component: row => <>
            <Tooltip content={row.estado ? t("pro.table.close") : t("pro.table.active")} placement="top">
                <><span className={`bp5-icon bp5-icon-folder-${row.estado ? 'close' : 'open'} text-${row.estado ? 'success' : 'danger'}`} /></>
            </Tooltip>
            {` ${row.codigo}`}
        </>
        },
